...
class DesmyDocumentPicker extends Component<Props, State> {
  ...
  handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      ...
      toast.error("Only Microsoft Word documents (.doc, .docx) are allowed.");
      return;
    }
      ...
      <div>
        {this.state.input.file ? (
          <div className="flex w-full space-x-2 dark:text-white bg-primary/30 rounded-lg px-3 py-2 relative">
            <div>
              <img src={'/static/word.png'} className="w-8" alt="Word Document" />
            </div>
          ...
          <div className="flex">
            <label htmlFor="fileInput" className="block text-sm text-slate-500 rounded-full dark:text-white file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-violet-50 dark:file:bg-darkDialogBackground file:text-primary dark:file:text-white hover:file:bg-violet-100 focus:outline-none">
            ...
            <input type="file" id="fileInput" accept=".doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={this.handleFileChange} className="hidden" />
          </div>
        )}
      </div>
    );
