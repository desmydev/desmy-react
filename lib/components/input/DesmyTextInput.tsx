    ...
    handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        try {
            ...
            if (type === State.PHONE) {
                ...
                this.setState({ input }, () => {
                    if (type === State.EMAIL) {
                        const email_extension = Commons.toStringDefault(this.props.email_contains, "");
        ...
        return (
            <div className="bg-white dark:bg-slate-900 p-4">
                <div className="relative bg-inherit">
                    ...
                    <label htmlFor="data" className="absolute cursor-text left-0 -top-3  text-[11px] dark:text-white bg-inherit mx-1 px-2 peer-placeholder-shown:text-sm  dark:peer-placeholder-shown:text-white peer-placeholder-shown:top-3 peer-focus:-top-3 peer-focus:text-black dark:peer-focus:text-white peer-focus:text-[11px] transition-all">{this.props.label}</label>
                </div>
            </div>
        );
    }
